...
class DDPG:
	"""docstring for DDPG"""
	def __init__(self, env, N_NEURONS):
		"__init__"
		self.name = 'DDPG' # name for uploading results
		self.environment = env
		# Randomly initialize actor network and critic network
		# with both their target networks
		self.state_dim = N_NEURONS #env.observation_space.shape[0]
		self.action_dim = N_NEURONS #env.action_space.shape[0]
	...
	def train(self):
		"trains actor, critic networks using DPG algorithm"
	...
	def action_activations(self, state):
		"returns action AND activations"
		action, activations = self.actor_network.action_activations(state)
		return action, activations

	def perceive(self, state, action, reward, next_state, done):
		"stores transition into replay buffer, trains network"
