...
class Neuron:
	...
	def __init__(self,state_dim,  action_dim ):
		...
		self.actor_weights = self.init_var(state_dim)
		self.q_weights = self.init_var(state_dim)
		self.q_weightsTarget = self.init_var(state_dim)
		self.actor_weightsTarget = self.init_var(state_dim)
		self.t = 0
		np.random.seed(0)
	def init_var(self, shape=[1]):
		a = np.array(2* np.random.rand(*shape) - 1)
		return a
	...
	def updateTargets(self):
		...
		self.t += 1
		# if self.t % 10000 == 0:
		# 	self.q_weightsTarget = self.q_weights
		# 	self.actor_weightsTarget = self.actor_weights
		self.q_weightsTarget += (1 - TAU) * (self.q_weights - self.q_weightsTarget)
		self.actor_weightsTarget += (1-TAU) * (self.actor_weights - self.actor_weightsTarget) 

	def getAction(self, state):
		return np.tanh(np.dot(self.actor_weights, state))

	def getExplore(self, state):
		return np.random.rand([1]) * 2 - 1

	def getQ(self, action, state):
		muOffPolicy = np.tanh(np.dot(state, self.actor_weightsTarget))
		muGrads = np.dot(state, 1 - np.square(muOffPolicy))
		return np.dot(np.dot(action - muOffPolicy, muGrads), self.q_weights), muOffPolicy, muGrads

	def getTargetQ(self, action, state):
		muOffPolicy = np.tanh(np.dot(state, self.actor_weightsTarget))
		muGrads = np.dot(state, 1 - np.square(muOffPolicy))
		return np.dot(np.dot(action - muOffPolicy, muGrads), self.q_weightsTarget), muOffPolicy, muGrads
	...
	def trainCritic(self, reward, action, state, nextaction, nextstate, done):
		...
		if agradient < -1:
			agradient = -1
		self.q_weights -= critic_learning_rate * agradient

	def trainActor(self, action, state):
		q, mu, muGrads = self.getQ(action, state)
		weightgradient = np.dot(np.dot(muGrads, muGrads), self.q_weights)
		if np.linalg.norm(weightgradient) > 1:
			weightgradient = 1
		if weightgradient < -1:
			weightgradient = -1
		#print("t: ",self.t, " ", self.actor_weights)
		self.actor_weights += actor_learning_rate * weightgradient
