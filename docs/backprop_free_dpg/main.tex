...
For a policy $\pi$  the action-value function is the expected future reward under $\pi$ by performing $a_t$ at state $s_t$. A temporally local definition thereof can be obtained using the Bellman equation
\begin{equation}
...
\begin{itemize}
  ...
  \begin{equation}
    T(v_t, \alpha_t) = (I - \chi_{n,n}) D(v_t, \epsilon(s_t)) + e_n \alpha
  \end{equation}
  where $e_n$ is the $n^{th}$ unit basis vector, $I$ is the identity, and $\chi_{n,n} =1$ if $n = n$ and $0$ otherwise. Intuitively, the transition in $E^n$ is the normal neural dynamics on $\scriptn$ except for at the neuron $n$, itself; in $E^n$ we set the voltage of $n$ in $v_{t+1}$ to be the voltage chosen, $\alpha_t$.
\end{itemize}
 Lastly an agent  $\mu^n: \scriptv \to \mathbb{R}$  is called \emph{neuromorphically local} to $\scriptn$ if $v_t \mapsto \langle D(v_\tau, \epsilon(s_t)), e_n \rangle$; that is, $\mu^n$ acts according to the normal dynamics.


We now can view every neuron in $\scriptn$ as an agent in its own environment, acting on its anterior neurons, and we can extend the action-value definition to $\mu^n$ as follows
\begin{equation}
    Q^{\mu^n}(v_t, \alpha_t) = \expect_{v_{t+1}\ \sim E^n}\left[r(v_t, a_t) + \gamma Q^{\mu^n}(v_{t+1}, \mu^n(v_{\tau+1}))\right].
\end{equation}
