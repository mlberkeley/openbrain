...
Now let $E$ and $\scriptn$ be defined as above. If $n$ is some neuron in $\scriptn$, assume that the voltage of $n$ is a function of $\scriptv$.Then define the a deterministic \emph{sub-environment} $E^n = (\scriptv, \mathbb{R}, \scriptr, T^n, r^n)$. In this case the transition describes how the voltage of $n$ affects the voltage of $\scriptn$ that is, 
\begin{equation}
	T\left(V(\tau+1) \suchthat V(\tau) \in \scriptv,   V(\tau+1)_n = a^n_\tau  \in \mathbb{R}\right) = 1.
\end{equation}
In other words the voltage of the neuron $n$ at time step $\tau +1$ is a function of $V(\tau)$ and the new state presented is $V(\tau+1)$ such that the new voltage of $n$ is $a^n_\tau$.
 The reward function is $r^n(V(\tau +\ell), a_{\ell + \tau}) = r_t(s_t, \mu(s_t))$ if $s_t$ presented at $\tau$ and $\mu(s_t)$ decodes $\scriptn$ at time $\tau + \ell$, otherwise $r^n(v,a) = 0.$

Now define an \emph{neuromorphically local agent} $\mu^n: \scriptv \to \mathbb{R}$ so that $V(\tau) \mapsto\langle \sigma(K\Theta[V(\tau)]) + I(\tau), e_n\rangle$ where $e_n$ is the basis vector for the $n^{th}$ neuron's voltage. Immediately we can extend the action-value function to this sub-environment and agent,
\begin{equation}
	Q^{\mu^n}(v_\tau, a_\tau^n) = \expect_{V(\tau+1)\ \sim E^n}\left[r(v_\tau, a_\tau^n) + \gamma Q^{\mu^n}(V(\tau+1), \mu^n(V(\tau+1)))\right].
\end{equation}

Provided with the previous definitions, the following question arises: does deterministic policy gradient learning on $\scriptn$, specifically $\mu$ on $E$, \emph{commute} with performing the same operation simultaneously on every neuromorphically local agent $\mu^n$ comprising $\scriptn$ and their respective sub-environments $E^n$? Supposing that we have the true $Q^\mu$ function and $\mu$ is optimal with respect to $Q^\mu$, then it is intuitive, but not obvious, that every $\mu^n$ should behave optimally with respect to an infinite time horizion, but will reverse hold? In answer to this question, we propose the following conjecture.
